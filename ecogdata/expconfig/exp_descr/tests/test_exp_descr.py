...
def test_rolled_shape2():
    ...
    assert exp.rolled_conditions_shape() == t_sizes
    exp.set_enum_tables(('b', 'a'))
    assert exp.rolled_conditions_shape() == t_sizes[::-1]
...
def test_rolled_shape3():
    ...
    for i, j, k in itertools.permutations([0, 1, 2]):
        ...
        assert exp.rolled_conditions_shape() == (ts[i], ts[j], ts[k])
...
def test_stim_exp_iterator():
    ...
    for n, (mask, sl) in enumerate(exp.iterate_for('a', c_slice=True)):
        assert sl == [n, slice(None), slice(None)]
        all_mask += mask.astype('i')
    assert (all_mask == 1).all()
...
def test_enumerate_condition():
    ...
    assert (np.take(conds.a, seq) == exp.a).all(), 'Enumeration order wrong 1d'

    exp, combos, tables = _gen_exp(n_cond=2)
    exp.set_enum_tables(('a', 'b'))
    seq, conds = exp.enumerate_conditions()
    assert (np.take(conds.a, seq) == exp.a).all(), 'Enumeration order wrong 2d'
    assert (np.take(conds.b, seq) == exp.b).all(), 'Enumeration order wrong 2d'

    exp, combos, tables = _gen_exp(n_cond=3)
    exp.set_enum_tables(('a', 'b', 'c'))
    seq, conds = exp.enumerate_conditions()
    assert (np.take(conds.a, seq) == exp.a).all(), 'Enumeration order wrong 3d'
    assert (np.take(conds.b, seq) == exp.b).all(), 'Enumeration order wrong 3d'
    assert (np.take(conds.c, seq) == exp.c).all(), 'Enumeration order wrong 3d'
