...
def quick_tiles(n_frames, nrow=None, ncol=None, **kws):
    """Quickly build tiled axes with no particular order."""
    if not (nrow or ncol):
        ncol = 4
    ...
    fig, axes, _ = tiled_axes((nrow, ncol), np.arange(n_frames), col_major=False, **kws)
    return fig, axes


def tiled_axes(geo, p, tilesize=(1, 1), col_major=True, title='', calib='none'):
    """
...
def calibration_axes(ref_ax, y_scale=None, t_scale=None, calib_ax=None, calib_unit='V', time_units='ms', fontsize=11):
    """
...
def fill_null(ax_list, cmap='gray'):
    """Draw a gray checker for an unused grid axes."""
    for ax in ax_list:
        ax.imshow(
...
def tile_images(maps, geo=(), p=(), col_major=True, border_axes=False, title='', fill_empty=False, fill_cmap='gray',
                x_labels=(), y_labels=(), clabel='none', **imkw):
    # NOT WELL TESTED?
...
def tile_traces(traces, geo=(), p=(), yl=(), twin=(), plot_style='sample', border_axes=False, col_major=True,
                title='', tilesize=(1, 1), calib_unit='V'):
    # *** tile_traces_1ax should be used instead ***
    p = _build_map(p, geo, col_major)
    if not len(p):
        # no permutation
    ...
    if not len(missed) or calib_unit is None:
        return fig
...
def tile_traces_1ax(traces, geo=(), p=(), yl=(), twin=(), plot_style='sample', col_major=True, title='',
    ...
    if plot_style != 'all':
        ...
        lines = [np.c_[tx + t0, mn + y0] for mn, (t0, y0) in zip(chan_means, chan_offsets)]
        line_kws.setdefault('linewidths', 2 if plot_style == 'single' else 1)
        line_kws.setdefault(
            'colors',
            dict(sem='r', stdev='k', sample='k', single='b')[plot_style]
        )
        lines = LineCollection(lines, **line_kws)
    else:
        lines = list()
        for chan_samps, (t0, y0) in zip(traces, chan_offsets):
            lines.extend([np.c_[tx + t0, samp + y0] for samp in chan_samps])
    ...
    if twin[0] < 0 and twin[1] > 0:
        vert_lines = [np.array([[t0, yl[0] + y0], [t0, yl[1] + y0]]) for t0, y0 in chan_offsets]
        trig_lines = LineCollection(
            vert_lines, colors='k', linestyles='--', linewidths=0.5
        )
