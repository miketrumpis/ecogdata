...
def embedded_frames(frames, geometry, gap=0.05, border=0.05, fill=np.nan):
    ...
    gap, border = map(
        lambda x: round(x * (f_row * m + f_col * n) / 2.0), (gap, border)
    )
...
def label_multi_axes(
    ...
    if bottom:
        b_ax = filter(lambda x: x.is_last_row(), fig.axes)
        if not len(b_ax):
            b_text = 0
        ax = b_ax[0]
    ...
    if left:
        l_ax = filter(lambda x: x.is_first_col(), fig.axes)
        if not len(l_ax):
            l_text = 0
        l_text = []
        for ax in l_ax:
    ...
    if top:
        t_ax = filter(lambda x: x.is_first_row(), fig.axes)
        if not len(t_ax):
            t_text = 0
        t_text = [len(ax.get_title()) > 0 for ax in t_ax]
        t_text = np.any(t_text)
    if right:
        r_ax = filter(lambda x: x.is_last_col(), fig.axes)
        if not len(r_ax):
            r_text = 0
        r_text = []
        for ax in r_ax:
            if ax.yaxis.get_ticks_position() == u'left':
                continue
