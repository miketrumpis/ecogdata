...
def loadFolderToArray(folderpath, channels='all', dtype=float, 
    ...
    """
    # Get list of files
    filelist = get_filelist(folderpath, source,
                            channels=channels, recording=None, **kwargs)
...
def loadFolderToTransArray(folderpath, channels='all', dtype=float, 
    source='100', recording=None, start_chan=None, stop_chan=None,
    verbose=True, **kwargs):
    """Load the neural data files in a folder to a single transpose array.
    
    This method operates similarly to loadFolderToArray, but loads
    entire channels (in the specified range). This method allows
    flat-file writing in (channel, sample) order.
    ...
    Args:
        ...
        recording : int, or None
            ...
            incrementing label. For the first or only recording, leave this as
            None. Otherwise, specify an integer.
        start_chan, stop_chan : the first and last channel to read from
            each file. Python indexing is used, so 
            `stop_chan` is not inclusive. If `start_chan` is None, 
            start at the beginning; if `stop_chan` is None, read to the end.
        verbose : print status updateds
...
def pack(folderpath, filename='openephys.dat', dref=None,
    chunk_size=4000, start_record=None, stop_record=None,
    transpose=False, dtype=np.int16, verbose=True,
    **kwargs):
    ...
    Args:
        ...
        filename : name of file to store packed binary data
            ...
            MT EDIT: filename is fully-specified path
    ...
    # start/stop record will be interpreted according to packing layout:
    # * will refer to records if packing (sample, channel)
    # * will refer to channels if packing (channel, sample)
    ...
    for chunk_start in range(start_record, stop_record, chunk_size):
        ...
        # Load the chunk
        if transpose:
            data_array = loadFolderToTransArray(
                folderpath, dtype=dtype, verbose=False,
                start_chan=chunk_start, stop_chan=chunk_stop, **kwargs
                )
        else:
            data_array = loadFolderToArray(
                folderpath, dtype=dtype, verbose=False,
                start_record=chunk_start, stop_record=chunk_stop, **kwargs
                )
        ...
        if not is_file:
            ...
            print(filename.tell())
    if warn_dref:
        print('Warning: re-referencing not performed in transpose mode')
...
def _get_sorted_channels(folderpath, ctype='CH', recording=None):
    ...
    ctype : str
        Default is get the 'CH' channel files. Also can get 'ADC', 'AUX'
    ...
    if recording is None:
        return sorted( [int(f.split('_'+ctype)[1].split('.')[0])
                        for f in os.listdir(folderpath) 
                        if '.continuous' in f and '_'+ctype in f] ) 
    else:
        ...
        regex_pattern = '%s_%s(\d+)%s.continuous' % ('100', ctype, recording_s)
...
def get_channel_bytes(filepath):
    """Get channel length in bytes"""
    with open(filepath, 'rb') as f:
        file_length = os.fstat(f.fileno()).st_size
    return file_length

from glob import glob
def get_filelist(folderpath, source='100', ctype='CH', channels='all', recording=None):
    ...
    if channels == 'all': 
        channels = _get_sorted_channels(
            folderpath, ctype=ctype, recording=recording
            )

    g_fn = lambda f: glob(os.path.join(folderpath, '*'+f+'*'))[0]
    # Get the list of continuous filenames
    if recording is None or recording == 1:
        # The first recording has no suffix
        filelist = [ g_fn('%s_%s%d.continuous' % (source, ctype, chan))
...
def get_header_from_folder(folderpath, filelist=None, **kwargs):
    ...
    unique_header = {}
    for key in included_keys + included_float_keys + included_timestamp_keys:
    ...
    for header in header_l:
        ...
        # Check timestamp (allow 1-point difference)
        t1 = unique_header['date_created'].strip("'")
        t2 = header['date_created'].strip("'")
        if np.abs( int(t1.split(' ')[-1]) - int(t2.split(' ')[-1]) ) > 1:
            raise ValueError("inconsistent file creation timestamps")
